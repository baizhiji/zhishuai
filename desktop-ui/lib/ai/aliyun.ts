 ...
 * 说明: 未配置 API Key 或调用失败时直接抛错，禁止返回模拟数据。
 */
...
class AliyunBailianClient {
  ...
  constructor() {
    ...
    if (!this.apiKey) {
      console.warn('阿里云百炼 API Key 未配置（NEXT_PUBLIC_ALIYUN_API_KEY），AI 功能将不可用');
    }
  }
  ...
  async generateText(request: TextGenerationRequest): Promise<TextGenerationResponse> {
    if (!this.apiKey) {
      throw new Error('阿里云百炼 API Key 未配置，请在环境变量中设置 NEXT_PUBLIC_ALIYUN_API_KEY');
    }
    ...
    if (!response.ok) {
      throw new Error(`阿里云百炼 API 请求失败: ${response.status} ${response.statusText}`);
    }
