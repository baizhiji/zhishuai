...
const sections = [
  ...
  },
  {
    title: '二、我们收集的信息',
    content:
      '在您使用智枢AI服务的过程中，我们可能收集以下类别信息：1）账号信息：您注册时提供的手机号、姓名、密码（加密存储）及头像等资料；2）使用信息：您的登录日志、操作记录、功能使用情况、IP 地址、设备型号与系统版本等；3）内容信息：您通过平台创作、上传、发布的内容，包括文字、图片、音频、视频及其元数据；4）第三方平台信息：当您使用矩阵管理、一键发布等功能授权绑定第三方内容平台账号时，我们可能获取该等平台账号的公开信息及发布权限；5）AI 交互信息：您与 AI 助手对话中输入的内容以及 AI 生成的输出结果。',
  },
  {
    title: '三、信息的使用目的',
    content:
  ...
  return (
    <div style={{ maxWidth: 860, margin: '0 auto', padding: '48px 24px 64px' }}>
      ...
      <p style={{ color: '#999', fontSize: 14, marginBottom: 40 }}>
        更新日期：2026 年 8 月 13 日 &nbsp;|&nbsp; 生效日期：2026 年 8 月 13 日
      </p>
