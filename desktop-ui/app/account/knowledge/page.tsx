...
export default function KnowledgeBasePage() {
    ...
    { title: '上传时间', dataIndex: 'createdAt', key: 'createdAt' },
    {
      ...
      render: (_: any, record: Document) => (
        <Space>
          ...
          <Popconfirm
            title="确定删除？"
            onConfirm={() => setDocuments(documents.filter(d => d.id !== record.id))}
          >
        ...
        setKnowledgeList(
          knowledgeList.map(k =>
            k.id === editingKnowledge.id
              ? { ...k, ...values, updatedAt: new Date().toISOString().split('T')[0] }
              : k
          )
        );
  ...
  const filteredData = knowledgeList.filter(
    k => !searchText || k.name.includes(searchText) || k.description.includes(searchText)
  );
          ...
          <div>
            <Row gutter={[16, 16]} className="mb-4">
              ...
              <Col span={12}>
                ...
                <Tag>{selectedKnowledge.type === 'auto' ? '自动学习' : '手动录入'}</Tag>
              </Col>
              <Col span={12}>
                <Text type="secondary">文档数：</Text>
                <Text>{selectedKnowledge.docCount}</Text>
              </Col>
              <Col span={12}>
                <Text type="secondary">字数：</Text>
                <Text>{selectedKnowledge.wordCount.toLocaleString()}</Text>
              </Col>
              <Col span={12}>
                <Text type="secondary">创建时间：</Text>
                <Text>{selectedKnowledge.createdAt}</Text>
              </Col>
              <Col span={12}>
                <Text type="secondary">更新时间：</Text>
                <Text>{selectedKnowledge.updatedAt}</Text>
              </Col>
            </Row>
            <div className="mb-4">
            ...
            <div>
              ...
              <Progress
                percent={
                  selectedKnowledge.status === 'active'
                    ? 100
                    : selectedKnowledge.status === 'training'
                      ? 65
                      : 0
                }
                status={selectedKnowledge.status === 'active' ? 'success' : 'active'}
                className="mt-2"
              />
            </div>
          </div>
