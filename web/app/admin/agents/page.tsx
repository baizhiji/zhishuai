...
export default function AdminAgentsPage() {
  ...
  const loadAgents = async () => {
    try {
      setLoading(true);
      const res = await request.get<ApiResponse<{ data: Agent[]; pagination: any }>>(
        '/admin/agents',
        {
          params: {
  ...
  const handleCreate = async () => {
    try {
      ...
      const typeText =
        agentType === 'one_time'
          ? `一次性付费 ¥${oneTimeFee || 0}`
          : `按比例分成 ${commissionRate || 20}%`;
      message.success(
        `已开通代理商：${values.name}，${typeText}，登录账号：${values.phone}，初始密码：123456`
      );
    ...
    try {
      ...
      if (res.data) {
        setSelectedAgent({
          ...agent,
          ...res.data.data,
          createTime: (res.data as any).data.user?.createdAt
            ? dayjs((res.data as any).data.user.createdAt).format('YYYY-MM-DD')
            : '',
  ...
  const columns: ColumnsType<Agent> = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <div>
          <div>
            当月: <Text style={{ color: '#52c41a' }}>¥{record.monthPaid || 0}</Text>
          </div>
          <div>
            累计: <Text style={{ color: '#fa8c16' }}>¥{record.totalPaid || 0}</Text>
          </div>
        </div>
      ),
    },
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      <Row gutter={16} style={{ marginBottom: 24 }}>
        ...
        <Col span={6}>
          <Card loading={loading}>
            <Statistic title="正常代理商" value={stats.active} valueStyle={{ color: '#52c41a' }} />
          </Card>
        </Col>
        <Col span={6}>
          <Card loading={loading}>
            ...
            locale={{
              emptyText: (
                <Empty
                  ...
                  description={<span>{searchText ? '未找到匹配的代理商' : '暂无代理商数据'}</span>}
                >
                  {!searchText && (
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item name="region" label="负责区域（省/市）">
                <Cascader options={cascaderOptions} placeholder="选择省/市" changeOnSelect />
              </Form.Item>
            </Col>
            <Col span={12}>
              <Form.Item
          ...
          <Row gutter={16}>
            <Col span={12}>
              ...
              >
                <Select
                  placeholder="选择代理商类型"
                  onChange={() => createForm.setFieldValue('commissionRate', undefined)}
                >
          ...
          <>
            <Descriptions column={2} bordered size="small">
              ...
              <Descriptions.Item label="代理等级">
                {selectedAgent.level === 'province'
                  ? '省级代理'
                  : selectedAgent.level === 'city'
                    ? '市级代理'
                    : '区/县代理'}
              </Descriptions.Item>
              ...
              <Descriptions.Item label="到期时间">
                {selectedAgent.expireAt
                  ? dayjs(selectedAgent.expireAt).format('YYYY-MM-DD')
                  : '永久'}
              </Descriptions.Item>
