...
for params in \
  "{\"model\":\"yt-video-humanactor\",\"image\":\"${IMAGE}\",\"audio\":\"${AUDIO}\",\"duration\":10}" \
  "{\"model\":\"yt-video-humanactor\",\"image_url\":\"${IMAGE}\",\"audio_url\":\"${AUDIO}\",\"prompt\":\"数字人口播\"}" \
  "{\"model\":\"yt-video-humanactor\",\"image_url\":\"${IMAGE}\",\"audio_url\":\"${AUDIO}\",\"text\":\"欢迎来到智枢AI\"}" \
  "{\"model\":\"yt-video-humanactor\",\"reference_image_url\":\"${IMAGE}\",\"audio_url\":\"${AUDIO}\",\"duration\":10}"
do
  echo ""
  echo "=== params: $params ==="
