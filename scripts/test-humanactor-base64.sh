...
# 下载一张小图并转 base64
IMAGE_URL="https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=512&h=512&fit=crop"
TMP="/tmp/human_test.jpg"
...
echo ""
echo "=== 数字人 with base64 image ==="
curl -s -w "\nHTTP %{http_code}\n" -X POST "${TBASE}/api/video/submit" \
  -H "Authorization: Bearer ${TKEY}" \
  -H "Content-Type: application/json" \
  -d "{\"model\":\"yt-video-humanactor\",\"image_url\":\"${DATA_URL}\",\"audio_url\":\"${AUDIO}\"}" | tail -10
