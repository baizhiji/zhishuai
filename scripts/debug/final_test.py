...
# Login with proper JSON escaping
login_body = json.dumps({"phone":"18601655222","password":"123456"})
login_result = run(f"curl -s -X POST http://localhost:3001/api/auth/login -H 'Content-Type: application/json' -d '{login_body}'")
...
print(f'\nToken: {token[:30]}...')
print(f'Login result: {login_result[:200]}')

if not token:
    print('LOGIN FAILED - trying alternative approach')
...
print('\n' + '='*60)
print('VERIFICATION WITH CORRECT TOKEN')
print('='*60)

# Health
r = run('curl -s https://baizhiji.net/api/health')
print(f'[Health] {"PASS" if "ok" in r else "FAIL"}')

if token:
    # AI Chat
    chat_body = json.dumps({"messages":[{"role":"user","content":"Hello"}],"modelKey":"qwen_turbo"})
...
# PM2
r = run('pm2 list 2>&1')
print(f'[PM2] {"PASS" if "online" in r else "FAIL"}')

# Setup PM2 startup
run('sudo env PATH=$PATH:/usr/bin /usr/lib/node_modules/pm2/bin/pm2 startup systemd -u ubuntu --hp /home/ubuntu 2>&1 | tail -3')
run('pm2 save 2>&1')
