...
echo ""
echo "=== 数字人 with prompt ==="
curl -s -w "\nHTTP %{http_code}\n" -X POST "${TBASE}/api/video/submit" \
  -H "Authorization: Bearer ${TKEY}" \
  -H "Content-Type: application/json" \
  -d "{\"model\":\"yt-video-humanactor\",\"prompt\":\"一位年轻女性微笑着讲话，背景为简洁办公室\",\"image_url\":\"${IMAGE}\",\"audio_url\":\"${AUDIO}\",\"duration\":10}" | tail -10

echo ""
echo "=== 数字人 no image, with prompt + audio ==="
curl -s -w "\nHTTP %{http_code}\n" -X POST "${TBASE}/api/video/submit" \
  -H "Authorization: Bearer ${TKEY}" \
  -H "Content-Type: application/json" \
  -d "{\"model\":\"yt-video-humanactor\",\"prompt\":\"一位年轻女性微笑着讲话\",\"audio_url\":\"${AUDIO}\",\"duration\":10}" | tail -10
