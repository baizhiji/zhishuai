...
### 1.1 desktop-ui/（11 HIGH，CRITICAL 0，与 08-20 完全一致）

| 漏洞包 | 严重性 | 说明 | 修复方式 |
|--------|--------|------|----------|
| `next` 本体（直接依赖） | HIGH | 20+ 公告（最高 CVSS 8.6 SSRF GHSA-p9j2-gv94-2wf4，另含 DoS / XSS / Cache Poisoning） | 升级 next@16.3.1（破坏性，需回归测试） |
| `postcss`（next 内置 + 顶层） | HIGH | 路径遍历 .map 文件泄露 GHSA-r28c-9q8g-f849（CVSS 7.5）、任意文件读取 GHSA-6g55-p6wh-862q | 随 next@16.3.1 |
| `glob`（经 eslint-config-next） | HIGH | CLI 命令注入 GHSA-5j98-mcp5-4vw2（CWE-78，CVSS 7.5） | 升级 eslint-config-next@16.3.1（破坏性） |
| `eslint-config-next`（含 @next/eslint-plugin-next） | HIGH | 依赖 glob 命令注入链 | 升级 16.3.1（破坏性） |
...
### 1.2 server/（2 HIGH + 2 MODERATE，CRITICAL 0，与 08-20 完全一致）
...
|------|----------|------|----------|------|
| desktop-ui/ | 0 | 11 | 0 | 与 08-20 一致（连续 15 天 CRITICAL=0） |
| server/ | 0 | 2 | 2 | 与 08-20 一致 |

**合计：13 HIGH / 0 CRITICAL，连续第 13 天无新增高危漏洞。** 8 月 20 日至 21 日无新披露的 npm 高危公告。
...
| `ai-client.ts:196` | ❌ **CRITICAL（连续第 7 天）** | `ENCRYPTION_KEY` 硬编码回退 `'zhishuai-default-key-32chars!!'`，密钥公开导致 AES-256 加密形同虚设 |
| `user-api-key.service.ts:15` | ❌ **CRITICAL（连续第 7 天）** | 同上，同一默认密钥回退 |
| `admin-api-providers.ts:8-10` | ⚠️ 功能性风险 | API_KEY_ENCRYPTION_KEY 缺失时 `crypto.randomBytes(32)` 随机回退，进程重启后存量数据无法解密 |

**结论**：硬编码凭据无新增、无修复，遗留 1 处 CRITICAL（同一默认密钥出现在 2 个文件），连续第 7 天。

---

## 三、修复建议

npm 依赖层面 CRITICAL 已连续 15 天为 0，当前主要风险为 **ENCRYPTION_KEY 硬编码默认密钥回退**，属 CRITICAL 级。

### P0 — 密钥管理（人工确认后执行，已连续催促 7 天）

1. **fail-fast 改造**：将两处 `process.env.ENCRYPTION_KEY || 'zhishuai-default-key-32chars!!'` 改为缺失即抛错退出
   - `server/src/services/ai-client.ts:196`
   - `server/src/services/user-api-key.service.ts:15`
2. **密钥轮换 + 存量重加密**：生成新随机密钥写入生产 .env，编写幂等一次性重加密脚本，执行前备份数据库
...
| 破坏性升级 | 升级 next + eslint-config-next 至 16.3.1 | 需回归测试，消解 next 20+ HIGH 公告 |
...
依赖供应链平稳（CRITICAL=0 连续 15 天，HIGH 数量与昨日完全一致），无新增漏洞。主要风险已完全集中在密钥管理：**默认密钥回退使加密形同虚设（CRITICAL），已连续 7 天未处理**。P0 修复建议已完整写入本节，建议尽快安排人工确认执行密钥轮换。
