...
- 【3.6.1 发布完成】desktop 版本号 3.6.0→3.6.1（package.json / src-tauri/tauri.conf.json / Cargo.toml / next.config.js 四处）；commit 6d63f6d（修复）→ commit 4c003a4（版本号）推送 main 触发 CI；CI Deploy to Production success + Desktop Build 完成 → 服务器 downloads/zhishuai_3.6.1_x64-setup.exe(.sig) 已上传（3.67MB）；register_desktop_361.js 登记 AppVersion（archive 3.6.0 → insert 3.6.1 released），fix_361_releasedat.js 补 releasedAt；验证通过：/api/version/desktop/latest.json?currentVersion=3.6.0 → 返回 3.6.1（含 changelog+signature），下载 URL HTTP 200；用户打开 3.6.0 客户端即收到自动更新提示
- 【遗留】生产用户 13166262006（郝好）登录密码未知，未做 HTTP 层登录态验证（服务层 e2e 已验证）；代理商 13900000099 密码 123456 失效（verify-login 401，预期内）；register 脚本 INSERT AppVersion 时记得补 releasedAt=NOW()（否则 latest.json 排序取不到，见 fix_361_releasedat.js）
