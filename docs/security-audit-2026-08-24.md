...
### 1.1 desktop-ui/（11 HIGH，CRITICAL 0，数量与 08-21 一致）
...
### 1.2 server/（2 HIGH + 2 MODERATE，CRITICAL 0，与 08-21 一致）
...
|------|----------|------|----------|------|
| desktop-ui/ | 0 | 11 | 0 | 数量与 08-21 一致（连续 18 天 CRITICAL=0） |
| server/ | 0 | 2 | 2 | 与 08-21 一致 |

**合计：13 HIGH / 0 CRITICAL，连续第 16 个审计日无新增高危漏洞。**

**唯一变化**：npm 官方已发布 next / eslint-config-next **16.3.2** 修复版（08-21 时推荐版本为 16.3.1），修复建议目标版本同步更新。8 月 21 日至 24 日期间无新增披露的高危漏洞影响本依赖树。
...
| `ai-client.ts:206` | ❌ **CRITICAL（连续第 10 天）** | `ENCRYPTION_KEY` 硬编码回退 `'zhishuai-default-key-32chars!!'`，密钥公开导致 AES-256 加密形同虚设 |
| `user-api-key.service.ts:15` | ❌ **CRITICAL（连续第 10 天）** | 同上，同一默认密钥回退 |
| `admin-api-providers.ts` | ⚠️ 功能性风险 | API_KEY_ENCRYPTION_KEY 缺失时随机密钥回退，进程重启后存量数据无法解密 |

**结论**：硬编码凭据无新增、无修复，遗留 1 处 CRITICAL（同一默认密钥出现在 2 个文件），连续第 10 天。

---

## 三、修复建议

npm 依赖层面 CRITICAL 已连续 18 天为 0，当前主要风险为 **ENCRYPTION_KEY 硬编码默认密钥回退**，属 CRITICAL 级。

### P0 — 密钥管理（人工确认后执行，已连续催促 10 天）

1. **fail-fast 改造**：将两处 `process.env.ENCRYPTION_KEY || 'zhishuai-default-key-32chars!!'` 改为缺失即抛错退出
   - `server/src/services/ai-client.ts:206`
   - `server/src/services/user-api-key.service.ts:15`
2. **密钥轮换 + 存量重加密**：生成新随机密钥写入生产 .env，编写幂等一次性重加密脚本，执行前备份数据库
3. **admin-api-providers.ts 同步 fail-fast**：API_KEY_ENCRYPTION_KEY 缺失即启动报错，避免随机密钥导致重启后密文不可解
...
依赖供应链平稳（CRITICAL=0 连续 18 天，HIGH 数量与上次审计一致），8 月 21 日至 24 日无新增高危漏洞，仅 npm 官方发布 next 16.3.2 修复版使升级目标更新。主要风险已完全集中在密钥管理：**默认密钥回退使加密形同虚设（CRITICAL），已连续 10 天未处理**。P0 修复建议已完整写入本节，建议尽快安排人工确认执行密钥轮换。
